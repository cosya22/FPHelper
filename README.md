...
- **`plugins/telegram_stars`** — авто-выдача Telegram Stars через Fragment.com
  (официального способа пополнить чужой баланс Stars нет, поэтому используется
  сторонняя библиотека `pyfragment` и TON-кошелёк — заведите под это отдельный
  кошелёк, не основной). Бот сам просит у покупателя Telegram username в чате
  и выдаёт звёзды, а сверху — автопилот: периодически подтягивает цену звёзд с
  Fragment и курс TON/RUB, пересчитывает цену лота с вашей наценкой и обновляет
  её на FunPay; снимает лоты с продажи, если баланс кошелька ниже порога, и
  включает обратно, когда пополнили; напоминает покупателю подтвердить заказ,
  если тот получил звёзды, но не подтвердил спустя заданное время. Подробный
  гайд по установке: [telegra.ph/Telegram-Stars-dlya-FPHelper](https://telegra.ph/Telegram-Stars-dlya-FPHelper-08-14).
